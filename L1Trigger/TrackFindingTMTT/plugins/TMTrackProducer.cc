 ...
 void TMTrackProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
 {
  ...
  if(settings_->tracklet()){
   ...
   for (unsigned int iPhiSec = 0; iPhiSec < settings_->numPhiSectors(); iPhiSec++) {
    ...
    std::vector<std::vector<const Stub*>> barrelStubs (6); //barrel layer 0-5
    std::vector<std::vector<const Stub*>> diskStubs (5); // disk 0-4 (both + and - disks)
    Sector& sector = mSectors(iPhiSec, 0);//no eta regions in tracklet-tmtt
    sector.init(settings_, iPhiSec, 0); //no eta regions in tracklet-tmtt
    ...
    for (unsigned int innerSeedLayer = 0; innerSeedLayer < 5; innerSeedLayer++){
     for (unsigned int outerSeedLayer = 0; outerSeedLayer < 6; outerSeedLayer++){

      if((innerSeedLayer == 0 && outerSeedLayer==1) ||
        (innerSeedLayer == 2 && outerSeedLayer == 3) ||
        (innerSeedLayer == 4 && outerSeedLayer == 5) ||
        (innerSeedLayer == 1 && outerSeedLayer == 2) ){
    ...
    for (unsigned int innerSeedLayer = 0; innerSeedLayer < 5; innerSeedLayer++){
     for (unsigned int outerSeedLayer = 0; outerSeedLayer < 4; outerSeedLayer++){
    ...
    for (unsigned int innerSeedLayer = 0; innerSeedLayer < 2; innerSeedLayer++){
     for (unsigned int outerSeedLayer = 0; outerSeedLayer < 1; outerSeedLayer++){
